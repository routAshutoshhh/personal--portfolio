...
export function MailModal() {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof emailSchemaType, string>>
  >({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status != 200) {
        toast.error(
          "email send failed due to some error - please send a mail using gmail at the same email address"
        );
        throw new Error("Failed to send Email from the backend");
      }
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 overflow-auto">
            ...
            <div className="grid gap-3">
              ...
              />
              {isSubmitClicked && errors.message && (
